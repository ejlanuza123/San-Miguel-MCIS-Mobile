...
export default function BhwReportsScreen({ navigation }) {
    ...
    const quarterlyReports = useMemo(() => {
        ...
        const getQuarterData = (q) => {
            ...
            return {
                patients: allData.patients.filter(p => months.includes(new Date(p.created_at).getMonth())),
        ...
        return [1, 2, 3, 4].map(q => ({
            id: q,
            name: `BHW Report - ${year} ${q}${q === 1 ? 'st' : q === 2 ? 'nd' : q === 3 ? 'rd' : 'th'} Quarter`,
    ...
    const renderReportCard = ({ item }) => (
        <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('BhwViewReport', { quarter: item, allData: item.data })}>
