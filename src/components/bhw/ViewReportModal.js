...
export default function BhwViewReportScreen({ route, navigation }) {
  ...
  const reportStatus = allData.patients.length > 0 ? "Completed" : "Pending";

  const stats = useMemo(() => {
    const totalAppointments = allData.appointments.length;
    const completed = allData.appointments.filter(
    ...
    const highRiskCount = allData.patients.filter(
  ...
  const handleShareFile = async (data, fileName) => {
    ...
    try {
      const uri =
        FileSystem.documentDirectory + `${fileName.replace(/ /g, "_")}.csv`;
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Section title={quarter.name}>
          <Text style={styles.summaryText}>
            Summary of Prenatal Check-ups for Q{quarter.id}
          </Text>
        ...
        <Section title="Case Information">
          <InfoRow
            label="Case ID"
            value={`RPT-${quarter.year}-${quarter.id}`}
            valueColor="#374151"
          />
          <InfoRow
            label="Status"
            value={reportStatus}
            valueColor={reportStatus === "Completed" ? "#16a34a" : "#f59e0b"}
          />
          <InfoRow
            label="Assigned To"
        ...
        <Section title="Summary">
          <Text style={styles.paragraph}>
            This report covers maternal health activities during Q{quarter.id}{" "}
            {quarter.year}. A total of{" "}
            <Text style={{ fontWeight: "bold" }}>{stats.totalPatients}</Text>{" "}
            patients were involved, achieving a{" "}
        ...
        <Section title="Attachments">
          <TouchableOpacity
            ...
            onPress={() =>
              handleShareFile(allData.patients, "Maternity_Patient_Report")
            }
          >
            <FileIcon />
            <Text style={styles.attachmentText}>maternity_report.csv</Text>
            <Text style={styles.viewText}>View</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.attachmentRow}
            onPress={() =>
              handleShareFile(allData.appointments, "Appointments_Summary")
            }
          >
            <FileIcon />
            <Text style={styles.attachmentText}>appointments_summary.csv</Text>
            <Text style={styles.viewText}>View</Text>
          </TouchableOpacity>
        </Section>
        ...
        <Section title="Actions">
          <TouchableOpacity
            ...
            onPress={() =>
              handleShareFile(allData.patients, "Full_Patient_Report")
            }
          >
