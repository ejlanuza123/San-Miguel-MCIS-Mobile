// src/components/bns/BnsInventoryScreen.js
...
import ViewBnsInventoryModal from './ViewBnsInventoryModal';

// --- HELPER COMPONENTS ---
const StatusBadge = ({ status }) => {
    const statusInfo = { 'Normal': { backgroundColor: '#dcfce7', color: '#16a34a' }, 'Low': { backgroundColor: '#fef3c7', color: '#f59e0b' }, 'Critical': { backgroundColor: '#fee2e2', color: '#ef4444' } };
...
const StatusLegend = () => (
    <View style={styles.legendContainer}>
        <View style={[styles.legendTag, { backgroundColor: '#dcfce7' }]}><Text style={[styles.legendTagText, { color: '#16a34a' }]}>Normal</Text></View>
        <View style={[styles.legendTag, { backgroundColor: '#fef3c7' }]}><Text style={[styles.legendTagText, { color: '#f59e0b' }]}>Low</Text></View>
        <View style={[styles.legendTag, { backgroundColor: '#fee2e2' }]}><Text style={[styles.legendTagText, { color: '#ef4444' }]}>Critical</Text></View>
    </View>
);
...
const InventoryRow = ({ item, onPress }) => (
...
export default function BnsInventoryScreen() {
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            const options = [
                ...
                { label: 'Medicines', onPress: () => setActiveFilter('Medicines') },
                { label: 'Nutrition & Feeding', onPress: () => setActiveFilter('Nutrition & Feeding') },
                { label: 'Medical Supplies', onPress: () => setActiveFilter('Medical Supplies') },
                { label: 'Vaccines', onPress: () => setActiveFilter('Vaccines') },
                { label: 'Equipment', onPress: () => setActiveFilter('Equipment') },
                { label: 'Child Hygiene & Care', onPress: () => setActiveFilter('Child Hygiene & Care') },
    ...
    const fetchInventory = useCallback(async () => {
        ...
        let query = supabase.from('bns_inventory').select('*', { count: 'exact' });
        if (activeFilter !== 'All') query = query.eq('category', activeFilter);
        if (searchTerm) query = query.ilike('item_name', `%${searchTerm}%`);
        const { data, error, count } = await query.order('item_name', { ascending: true }).range(from, to);
    ...
    return (
        <>
            <Modal visible={isAddModalOpen} animationType="slide"><AddBnsInventoryModal onClose={() => setIsAddModalOpen(false)} onSave={fetchInventory} /></Modal>
            <Modal visible={isViewModalOpen} animationType="fade" transparent={true}><ViewBnsInventoryModal item={selectedItem} onClose={() => setIsViewModalOpen(false)} /></Modal>
            ...
            <View style={styles.container}>
                <View style={styles.mainCard}>
                    ...
                    <View style={styles.listHeader}>
                        ...
                        <Text style={[styles.headerText, styles.categoryCell]}>Category</Text>
                        <Text style={[styles.headerText, styles.stockCell]}>Stock</Text>
                        <Text style={[styles.headerText, styles.statusCell]}>Status</Text>
                        <Text style={[styles.headerText, styles.dateCell]}>Expire Date</Text>
                    </View>
                    {loading ? <ActivityIndicator size="large" color="#3b82f6" style={{flex: 1}}/> : (
                        <FlatList
                            data={inventory}
                            renderItem={({item}) => <InventoryRow item={item} onPress={() => { setSelectedItem(item); setIsViewModalOpen(true); }} />}
                ...
                <View style={styles.controlsContainer}>
                    <StatusLegend />
                    <TouchableOpacity style={styles.addButton} onPress={() => setIsAddModalOpen(true)}><Text style={styles.addButtonText}>+ Add New Item</Text></TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    mainCard: { flex: 1, backgroundColor: 'white', marginHorizontal: 20, marginTop: 20, borderRadius: 20, elevation: 3, overflow: 'hidden' },
    cardTitle: { fontSize: 24, fontWeight: 'bold', padding: 20, textAlign: 'center', color: '#111827' },
    ...
    row: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f3f4f6' },
    cell: { fontSize: 12, color: '#374151' },
    itemCell: { flex: 2.5, fontWeight: '600' },
    categoryCell: { flex: 2 },
    stockCell: { flex: 1.5, textAlign: 'center' },
    statusCell: { flex: 1.5, alignItems: 'center' },
