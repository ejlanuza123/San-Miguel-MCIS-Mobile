...
export default function BnsViewReportScreen({ route, navigation }) {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      completed,
      successRate,
      totalChildren: allData.children.length,
    };
  }, [allData]);
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Section title={quarter.name}>
          <Text style={styles.summaryText}>
            Summary of Child Health Activities for Q{quarter.id}
          </Text>
        ...
        <Section title="Case Information">
          ...
          />
          {(() => {
            const hasData = allData.children.length > 0;
            const status = hasData ? "Completed" : "Pending";
            const statusColor = hasData ? "#16a34a" : "#f59e0b";
            return (
              <InfoRow label="Status" value={status} valueColor={statusColor} />
            );
          })()}
          <InfoRow
            label="Assigned To"
        ...
        <Section title="Attachments">
          <TouchableOpacity
            ...
            onPress={() =>
              handleShareFile(allData.children, "Child_Health_Report")
            }
          >
            <FileIcon />
            <Text style={styles.attachmentText}>child_health_report.csv</Text>
            <Text style={styles.viewText}>View</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.attachmentRow}
            onPress={() =>
              handleShareFile(allData.inventory, "BNS_Inventory_Report")
            }
          >
            <FileIcon />
            <Text style={styles.attachmentText}>bns_inventory_report.csv</Text>
            <Text style={styles.viewText}>View</Text>
          </TouchableOpacity>
        </Section>
        ...
        <Section title="Actions">
          <TouchableOpacity
            ...
            onPress={() =>
              handleShareFile(allData.children, "Full_Child_Health_Report")
            }
          >
