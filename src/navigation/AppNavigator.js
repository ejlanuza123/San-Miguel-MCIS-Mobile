...
// NEW: Add RecordsIcon
const RecordsIcon = ({ color }) => ( <Svg width={28} height={28} viewBox="0 0 24 24"><Path fill={color} d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-1 12H9V5h10v9z"/></Svg> );
...
const BhwStack = () => (
    <Stack.Navigator screenOptions={{ header: () => <FixedHeader />, contentStyle: { backgroundColor: '#f0f4f8' } }}>
        ...
        <Stack.Screen name="BhwViewReport" component={BhwViewReportScreen} options={{ headerShown: false }} />
    </Stack.Navigator>
);
...
const BnsStack = () => (
    <Stack.Navigator screenOptions={{ header: () => <FixedHeader /> }}>
        ...
        <Stack.Screen name="BnsViewReport" component={BnsViewReportScreen} options={{ headerShown: false }} />
    </Stack.Navigator>
);
    ...
    <Tab.Navigator
        screenOptions={{
            ...
            tabBarStyle: {
                ...
                borderRadius: 15, height: 60, borderTopWidth: 0, elevation: 5,
            },
        }}
    >
        <Tab.Screen 
        ...
        <HeaderProvider>
            <Tab.Navigator screenOptions={({ route }) => ({
                ...
                tabBarIcon: ({ color }) => {
                    ...
                    if (route.name === 'Appointment') return <AppointmentIcon color={color} />;
                    if (route.name === 'Inventory') return <InventoryIcon color={color} />;
                    if (route.name === 'Reports') return <ReportsIcon color={color} />;
                },
            })}>
                <Tab.Screen name="Dashboard" component={UserStack} listeners={({ navigation }) => ({ tabPress: e => { e.preventDefault(); navigation.navigate('Dashboard', { screen: isBns ? 'BnsDashboard' : 'BhwDashboard' }); }, })} />
                <Tab.Screen name="Patient" component={UserStack} listeners={({ navigation }) => ({ tabPress: e => { e.preventDefault(); navigation.navigate('Patient', { screen: isBns ? 'ChildHealthRecords' : 'PatientManagement' }); }, })} />
                <Tab.Screen name="Appointment" component={UserStack} listeners={({ navigation }) => ({ tabPress: e => { e.preventDefault(); navigation.navigate('Appointment', { screen: isBns ? 'BnsAppointment' : 'BhwAppointment' }); }, })}/>
                <Tab.Screen name="Inventory" component={UserStack} listeners={({ navigation }) => ({ tabPress: e => { e.preventDefault(); navigation.navigate('Inventory', { screen: isBns ? 'BnsInventory' : 'BhwInventory' }); }, })} />
                <Tab.Screen name="Reports" component={UserStack} listeners={({ navigation }) => ({ tabPress: e => { e.preventDefault(); navigation.navigate('Reports', { screen: isBns ? 'BnsReports' : 'BhwReports' }); }, })} />
            </Tab.Navigator>
        </HeaderProvider>
    );
};
...
const AppNavigator = () => {
    return (
        <RootStack.Navigator screenOptions={{ headerShown: false }}>
            ...
            <RootStack.Screen name="QRScanner" component={QRScannerScreen} options={{ presentation: 'fullScreenModal' }} />
            ...
            <RootStack.Screen name="ViewBhwInventoryModal" component={ViewBhwInventoryModal} options={{ presentation: 'transparentModal', animation: 'fade' }} />
            <RootStack.Screen name="ViewBnsInventoryModal" component={ViewBnsInventoryModal} options={{ presentation: 'transparentModal', animation: 'fade' }} />
        </RootStack.Navigator>
    );
};
