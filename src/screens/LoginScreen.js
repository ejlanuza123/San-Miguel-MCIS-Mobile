...
import Animated, { FadeInDown, FadeInUp, FadeIn } from 'react-native-reanimated'; // <-- NEW
...
const EyeIcon = () => (
    <Svg width="20" height="20" viewBox="0 0 24 24" fill="none">
        <Path d="M12 4.5C7 4.5 2.73 7.61 1 12C2.73 16.39 7 19.5 12 19.5C17 19.5 21.27 16.39 23 12C21.27 7.61 17 4.5 12 4.5ZM12 15C10.34 15 9 13.66 9 12C9 10.34 10.34 9 12 9C13.66 9 15 10.34 15 12C15 13.66 13.66 15 12 15Z" fill="#9ca3af"/>
    </Svg>
);
const BackArrowIcon = () => (
...
const ErrorModal = ({ visible, onClose, message }) => {
    ...
    return (
        <View style={styles.modalOverlay}>
            <Animated.View style={styles.modalContainer} entering={FadeInUp.duration(300)}>
    ...
    const handleLogin = async () => {
        ...
        if (role === 'USER/MOTHER/GUARDIAN') {
            ...
            if (loginIdentifier.includes('@')) {
                ...
                const { data, error } = await supabase.rpc('get_email_by_user_id', { user_id_param: loginIdentifier });
                if (error || !data) {
        ...
        const { data: loginData, error: loginError } = await supabase.auth.signInWithPassword({ email: emailForLogin, password: password });
        if (loginError) {
        ...
        if (loginData.user) {
            ...
            if (profileError) {
                Alert.alert("Error", "Could not find a user profile. Please contact an administrator.");
                ...
                <SafeAreaView style={styles.background}>
                    <View style={styles.content}>
                        <Animated.Image 
                            ...
                            style={styles.logo} 
                        />
                        <Animated.View entering={FadeInUp.duration(800).delay(300)} style={styles.card}>
                            <Text style={styles.title}>Login</Text>
                            <View style={styles.inputContainer}>
                                <UserIcon />
                                <TextInput
                                    style={styles.input}
                                    placeholder={role === 'USER/MOTHER/GUARDIAN' ? 'Email Address' : 'User ID No.'}
